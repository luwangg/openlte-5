...
void* LTE_fdd_enb_radio::radio_thread_func(void *inputs)
{
          ...
          radio->is_started())
    {
        if(0 == radio_idx)
        {
            ...
            rx_radio_buf[buf_idx].current_tti = rx_current_tti; //1023*10-2
			...
			//等待1s, 并且时间戳是子帧的整数背后就可以进行发送接收操作了
            if(init_needed)
            {
                // Setup time specs
            ...
            if(!rx_synced)
            {
                ...
                /* 等待延时1s时间到达，等到接收消耗时间刚好为子帧时间的整数倍就说明同步了 */
                if(check_ts.to_ticks(samp_rate) == next_rx_ts.to_ticks(samp_rate))
                {
                    ...
                    /* 再接收一个子帧就超时了，如果在同步情况下不可能出现这种情况，因为1s中
                    *  包括整数个子帧，所以只会刚好到达或者差n个指针不会出现超出部分
                    */
					if(check_ts.to_ticks(samp_rate) > next_rx_ts.to_ticks(samp_rate))
                    {
                        /* 超时处理，同步 */
                        interface->send_debug_msg(LTE_FDD_ENB_DEBUG_TYPE_INFO,
                                                  ...
                                                  check_ts.to_ticks(samp_rate),
                                                  next_rx_ts.to_ticks(samp_rate));
						/* 找上个接收对应的时间戳，获取到达指定时间还剩多少个采样，然后在读取这么多个采样
						   校准接口操作等导致的延时
						*/
                        check_ts  -= uhd::time_spec_t::from_ticks(radio->N_rx_samps, samp_rate);
                ...
                /* 一次读取最小采样率对应的采样点数，然后进行时域同步，直到读取到一个子帧的采样数再处理 */

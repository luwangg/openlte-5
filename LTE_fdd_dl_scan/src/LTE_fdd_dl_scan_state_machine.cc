                                          ...
                                          gr_vector_void_star       &output_items)
{
    ...
    if(process_samples)
    {
              ...
              samp_buf_w_idx != 0)
        {
            switch(state)
            {
            case LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_COARSE_TIMING_SEARCH: /*  粗糙的搜索 */
                                                                                      ...
                                                                                      &timing_struct))
                {
                    ...
                    {
                        // Correct frequency error 一帧数据采样 校正指定峰值的频率误差(粗糙搜索时获得)
                        freq_shift(0, SAMP_BUF_SIZE, timing_struct.freq_offset[corr_peak_idx]);
            ...
            case LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_PSS_AND_FINE_TIMING_SEARCH:
                  ...
                  )
                {
                    ...
                    if(fabs(freq_offset) > 100)
                    {
                        ...
                        timing_struct.freq_offset[corr_peak_idx] += freq_offset; //保存频偏
                    }
        ...
        {
            samp_buf_r_idx -= 100; //多保存100个
            samps_to_copy   = samp_buf_w_idx - samp_buf_r_idx; //剩余
            samp_buf_w_idx  = 0;
...
void LTE_fdd_dl_scan_state_machine::channel_not_found(bool  &switch_freq, int32 &done_flag)
{
    LTE_fdd_dl_scan_interface *interface = LTE_fdd_dl_scan_interface::get_instance();

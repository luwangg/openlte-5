...
#define LIBLTE_PHY_N_SAMPS_CP_L_0_30_72MHZ    160 /* 第1个symbol的循环前缀采样数 */
#define LIBLTE_PHY_N_SAMPS_CP_L_ELSE_30_72MHZ 144 /* 其他6个symbol的采样数 */
...
typedef struct{
    // Receive   16: slot数量(2)*symbol(7~8)数  第0个是循环前缀
...
typedef struct{
    ...
    uint32 N_sc_rb_dl;
    uint32 N_sc_rb_ul;
    uint32 FFT_pad_size; //FFT多余的结果(FFT大小减去使用的载波数量)
    uint32 FFT_size;
    bool   ul_init;
}LIBLTE_PHY_STRUCT;
// Functions
...
// Structs
typedef struct{
    float  freq_offset[LIBLTE_PHY_N_MAX_ROUGH_CORR_SEARCH_PEAKS]; //每个峰值对应的频偏
    uint32 symb_starts[LIBLTE_PHY_N_MAX_ROUGH_CORR_SEARCH_PEAKS][7]; //每个峰值对应的7个symbol起始位置
    uint32 n_corr_peaks;
